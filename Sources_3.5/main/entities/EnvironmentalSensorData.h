...
class Flags {
  public:
    ...
    constexpr void set_source(Source src) noexcept {
        data_ = (data_ & ~Source_mask) |
                (static_cast<uint8_t>(src) << Source_shift);
    }
    ...
    constexpr void set_history(bool value) noexcept {
    ...
    constexpr uint8_t byte() const noexcept { return data_; }
    constexpr void set_byte(uint8_t value) noexcept { data_ = value; }
...
inline const char *parameters[paramsCount] = {
    "Temperature", "Humidity", "Pressure", "CO2", "VOC", "IAQ"};
